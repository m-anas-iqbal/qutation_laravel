...
@section('content')
    ...
    <?php
    if (isset($town->postcode)) {
        $pieces = explode(" ", $town->postcode);
        $pieces[0]; // piece1
        $pieces[1]; // piec
    }
    ?>
                ...
                <div class="col">
                    <nav aria-label="breadcrumb" style="padding-top: 80px">
                        <ol class="breadcrumb">
                            ...
                            @endif
                            @if(isset($country) && isset($state) && isset($county))
                            <li class="breadcrumb-item"><a href="{{ url('a/'.$country.'/'.$state.'/'.$county) }}">{{ $county }}</a></li>
                            @endif
                            @if(isset($country) && isset($state) && isset($county) && isset($city))
                            <li class="breadcrumb-item"><a href="{{ url('a/'.$country.'/'.$state.'/'.$county.'/'.$city) }}">{{ $city }}</a></li>
                            @endif
                            @if(isset($postcode_status))
...
//                                    $p[1]; // piec
                            ...
                            {{--<li class="breadcrumb-item">--}}
                                {{--<a href="{{ url('city-towns/'.$country.'/'.$state.'/'.$county.'/'.$name) }}">{{ $name }}</a></li>--}}
                                {{--@elseif(isset($country)  && isset($state) && $name)--}}
                                {{--<li class="breadcrumb-item"><a href="{{ url('county-cities/'.$country.'/'.$state.'/'. $name) }}">{{ $name }}</a></li>--}}
                                {{--@elseif(isset($country) && $name)--}}
                                {{--<li class="breadcrumb-item"><a href="{{ url('state-counties/'.$country.'/'. $name) }}">{{ $name }}</a></li>--}}
                                {{--@elseif($name)--}}
{{--                                <li class="breadcrumb-item"><a href="{{ url('country-states', $name) }}">{{ $name }}</a></li>--}}
                                <li class="breadcrumb-item active">{{ $name }}</li>
                            {{--@endif--}}
                            @endif
                            ...
                            @if(isset($town->postcode))
                                <li>
                                    <a href="{{ url('traders/postcode',$town->postcode) }}" style="margin-left: 35px;
    border: 2px solid gray;
    padding: -4px;
    padding-left: 10px;
    padding-right: 10px;
    border-radius: 20px;">{{ $pieces[0] }}</a></li>
                            @endif
                        </ol>
                    </nav>
                </div>
